...
const Login = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (!useJwt || typeof useJwt.login !== "function") {
        throw new Error("useJwt.login is not a function. Ensure useJwt is properly imported.");
      }
      ...
      if (!res.data) {
        console.error("API response does not contain expected 'data' field", res);
        setError("loginEmail", {
          type: "manual",
      ...
      if (!userData) {
        console.error("userData is missing in API response:", res.data);
        setError("loginEmail", {
          type: "manual",
      ...
      const loginData = { ...userData, accessToken, refreshToken };
      ...
      toast((t) => (
        <ToastContent
          ...
          name={loginData.fullName || loginData.username || "John Doe"}
        />
      ));
    } catch (err) {
      ...
      setError("loginEmail", {
        type: "manual",
        message: err?.response?.data?.error ?? "Login failed due to an unknown error.",
      });
    } finally {
      setLoading(false);
    }
